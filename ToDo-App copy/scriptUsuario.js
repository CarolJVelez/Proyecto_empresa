...
window.addEventListener('load', function () {
    ...
    devolverPrincipal.addEventListener('click', function () {
        ...
        document.querySelector('h2#usuariosTitle').style.display = 'none';
    });
    ...
    usuariosBtn.addEventListener('click', function () {
        if (usuariosContent.style.display === 'none') {
            ...
            document.querySelector('h2#usuariosTitle').style.display = 'block';         
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        productosBtn.addEventListener('click', function() {
            const productosEvent = new CustomEvent('productosButtonClick');
            document.dispatchEvent(productosEvent);
        });
    });
    ...
    form.addEventListener('submit', function (event) {
        ...
        verificarCorreoExistente(payload.email)
            .then(response => {
                if (response==true) {
                    alert("El correo electrónico ya está en uso. Por favor, utilice otro.");
                } else {
                    // El correo electrónico no está en uso, proceder con la creación del usuario
                    const settings = {
                        method: "POST",
            ...
            .catch(error => {
                alert("Ocurrió un error al verificar el correo electrónico. Por favor, inténtelo de nuevo más tarde.");
            });
    ...
    function verificarCorreoExistente(email) {
        console.log(email)
        return fetch(`http://localhost:8003/usuarios/verificar?email=${email}`)
            .then(response => {
                if (response) {
                    return true;
                } else {
